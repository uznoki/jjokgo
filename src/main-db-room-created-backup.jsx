...
import"./style.css";
const rooms=[["은호네 독서모임","어린 왕자",43,"17~19쪽"],["가족 책방","데미안",28,"4~6쪽"],["성당 천사부","아낌없이 주는 나무",67,"10~12쪽"]];
function App(){const[v,setV]=useState("home"),[room,setRoom]=useState(rooms[0]),[session,setSession]=useState(null),[loading,setLoading]=useState(true);
...
const openAuth=()=>setV("auth");return <div className="app"><main><header><b onClick={()=>setV("home")}>쪽<span>GO</span></b><button className="headAccount" onClick={()=>setV(session?"my":"auth")}><UserRound/>{session?"MY":"로그인"}</button></header>{loading?<div className="loading">쪽GO 불러오는 중…</div>:<>{v=="home"&&<HomeP setV={setV} open={r=>{setRoom(r);setV("room")}} session={session} openAuth={openAuth}/>} {v=="createRoom"&&<CreateRoom setV={setV} session={session}/>} {v=="rooms"&&<Rooms setV={setV} open={r=>{setRoom(r);setV("room")}} session={session} openAuth={openAuth}/>} {v=="room"&&<Room r={room} setV={setV}/>} {v=="my"&&(session?<My session={session} setV={setV}/>:<Auth setV={setV}/>)} {v=="auth"&&<Auth setV={setV}/>}</>}</main>{v!=="auth"&&<nav><N i={<Home/>} t="홈" f={()=>setV("home")}/><N i={<Users/>} t="함께 읽기방" f={()=>setV("rooms")}/><button className="mic" onClick={()=>alert("다음 단계에서 실제 녹음 서버 저장을 연결해요.")}><Mic/></button><N i={<BookOpen/>} t="내 서재"/><N i={<UserRound/>} t="MY 쪽GO" f={()=>setV(session?"my":"auth")}/></nav>}</div>}
...
function HomeP({setV,open,session,openAuth}){return <><section className="hero"><div><h1>한 쪽씩,<br/>함께 GO.</h1><p>오늘, 당신의 목소리로<br/>한 쪽을 이어주세요.</p></div><div>📖🌿</div></section>{!session&&<button className="welcome" onClick={openAuth}><UserRound/><span><b>쪽GO에 로그인해보세요</b><small>내 읽기 기록과 함께 읽기방을 이어갈 수 있어요.</small></span><ChevronRight/></button>}<div className="quick"><button className="green"><Mic/><b>오늘 한 쪽 읽기</b><small>공개 책에 참여하기</small></button><button onClick={()=>session?setV("rooms"):openAuth()}><Users/><b>함께 읽기방 만들기</b><small>우리만의 책 만들기</small></button></div><button className="join" onClick={()=>session?setV("rooms"):openAuth()}><Lock/> 초대 코드로 방 입장하기</button><h3>지금 가장 많이 읽히는 책</h3><div className="books">{[["어린 왕자",73],["데미안",58],["아낌없이 주는 나무",67]].map((x,i)=><div key={i}><div className="cover">📖</div><b>{x[0]}</b><strong>{x[1]}%</strong><Progress p={x[1]}/></div>)}</div><h3>나의 읽기 현황</h3><Calendar/><h3>참여 중인 방</h3><Card r={rooms[0]} f={()=>session?open(rooms[0]):openAuth()}/></>}
function Progress({p}){return <div className="bar"><i style={{width:p+"%"}}/></div>}
function Card({r,f}){return <button className="card" onClick={f}><div className="thumb">📖</div><span><b>{r[0]} 🔒</b><small>{r[1]} · 7명 참여</small><Progress p={r[2]}/></span><strong>{r[2]}%</strong><ChevronRight/></button>}
function Rooms({setV,open,session,openAuth}){return <><h1>함께 읽기방</h1><div className="tabs">참여 중인 방　　내가 만든 방</div>{rooms.map((r,i)=><Card key={i} r={r} f={()=>open(r)}/>)}<button className="wide" onClick={()=>session?setV("createRoom"):openAuth()}>+ 함께 읽기방 만들기</button><button className="join" onClick={()=>session?alert("다음 단계에서 초대코드를 실제 DB와 연결해요."):openAuth()}><Lock/> 초대 코드로 방 입장하기</button></>}
function Room({r,setV}){return <><button className="back" onClick={()=>setV("rooms")}>‹ 돌아가기</button><section className="room"><div className="thumb big">📖</div><div><h2>{r[0]} 🔒</h2><b>{r[1]}</b><p>7명 참여 · 함께 읽는 중</p></div></section><Progress p={r[2]}/><h3>페이지 배정</h3>{[["지현","17~19쪽"],["은호","20~22쪽"],["지호","23~25쪽"]].map((x,i)=><div className="assign" key={i}><b>{x[0]}</b><span>{x[1]} ›</span></div>)}<div className="due"><CalendarDays/><span>내 차례　<b>{r[3]}</b><br/>9월 12일까지</span></div><h3>최근 낭독</h3><div className="recent">▶　지현님이 17쪽을 낭독했어요</div><button className="wide"><Mic/> 내 낭독 시작하기</button></>}
function Calendar(){return <div className="cal">{["월","화","수","목","금","토","일"].map(x=><b key={x}>{x}</b>)}{[11,12,13,14,15,16,17].map((x,i)=><span key={x} className={i==1?"done":i==4?"doing":""}>{x}</span>)}</div>}
