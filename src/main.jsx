...
import clayHero from"./assets/jjokgo-clay-live-reading.png";
import"./style.css";
import"./calm.css";
import"./clay.css";
...
function HomeP({setV,session,openAuth}){const openRooms=()=>session?setV("rooms"):openAuth();return <><section className="hero"><div><small className="heroEyebrow">목소리로 이어지는 독서 시간</small><h1>한 쪽씩,<br/>함께 GO.</h1><p>같은 책을 펼치고, 서로의 목소리를 들으며<br/>오늘의 한 쪽을 함께 읽어요.</p></div><div className="heroArt" aria-hidden="true"><img src={clayHero} alt=""/><small>LIVE</small></div></section>{!session&&<button className="welcome" onClick={openAuth}><UserRound/><span><b>내 독서 기록을 이어가세요</b><small>로그인하면 읽기방과 기록을 안전하게 보관할 수 있어요.</small></span><ChevronRight/></button>}<section className="homeActions" aria-label="빠른 시작"><button className="primaryAction" onClick={openRooms}><span className="actionIcon"><Mic/></span><span><small>지금 바로 시작</small><b>LIVE 함께 읽기</b><em>실시간으로 목소리 나누기</em></span><ChevronRight/></button><button className="secondaryAction" onClick={openRooms}><Users/><span><b>함께 읽기방 만들기</b><small>책을 고르고 사람들을 초대해요</small></span><ChevronRight/></button><button className="codeAction" onClick={openRooms}><Lock/> 초대 코드로 입장</button></section><h3>지금 가장 많이 읽히는 책</h3><div className="books">{[["어린 왕자",73],["데미안",58],["아낌없이 주는 나무",67]].map((x,i)=><div className={`bookColor color-${i+1}`} key={i}><div className="cover">📖</div><b>{x[0]}</b><strong>{x[1]}%</strong><Progress p={x[1]}/></div>)}</div><h3>나의 읽기 현황</h3><Calendar/><h3>참여 중인 방</h3><Card r={demoRooms[0]} f={openRooms}/></>}
