  ...
  return <div className={`app view-${v}`}>
    ...
    <footer className="desktopFooter">
      ...
      <div>
        ...
        <button onClick={openModes}>LIVE ROOMS</button>
        <button onClick={openLibrary}>LIBRARY</button>
      </div>
      <b>READ TOGETHER, ONE PAGE AT A TIME.</b>
    </footer>
  </div>;
}
...
function HomeScheduleCalendar({session}){
  ...
  useEffect(()=>{
    ...
    fetchReadingSchedules(session.user.id)
      ...
      .catch(error=>{console.error("Home schedule load failed",error);if(active)setSchedules([])})
      .finally(()=>{if(active)setLoading(false)});
    return()=>{active=false};
  },[session?.user?.id]);
  return <Calendar schedules={schedules} diaryMessage={loading?"내 일정을 불러오는 중…":session?"등록된 일정이 없어요. MY에서 독서 일정을 등록해보세요.":"로그인하면 나의 쪽GO 일정을 여기에 표시할 수 있어요."}/>;
}
function Calendar({schedules=[],showDiary=true,diaryMessage="등록된 일정이 없어요."}){
  ...
  const visibleSchedules=schedules.filter(schedule=>schedule.is_active);
  ...
  return <section className="calendar" aria-label={`${year}년 ${month+1}월 읽기 현황`}>
    ...
    {showDiary&&<div className="scheduleDiary">
      ...
      {visibleSchedules.length?visibleSchedules.map(schedule=><article className="scheduleEntry" key={schedule.id}>
      ...
      </article>):<div className="scheduleDiaryEmpty">{diaryMessage}</div>}
    </div>}
  </section>
}
